"""
scripts/models/external_validation/halo_external_validation_on_acdb.py

Halo's external validation on the rest of the ACDB dataset (combinations that are not Bliss method
and were not in the Halo's training or test set)
"""
...
# ==========================
ext_pred_path = ext_out / "external_predictions_acdb.csv"
...
roc_df.to_csv(ext_out / "external_roc_curve_acdb.csv", index=False)
pr_df.to_csv(ext_out / "external_pr_curve_acdb.csv", index=False)
pr_thr_df.to_csv(ext_out / "external_pr_thresholds_acdb.csv", index=False)
...
cm_df.to_csv(ext_out / "external_confusion_matrix_acdb.csv")
