#!/usr/bin/env python3
"""
Random Forest baseline external validation for HALO on the Chandrasekaran
(INDIGO-source) dataset.

Mirrors the original halo_external_validation_on_chandrasekaran.py exactly,
except:
...
- feature selection (select_features_lgbm) is UNCHANGED -- still LightGBM-based,
  to keep this a controlled classifier-only comparison.
...
2. Load best hyperparameters from the RF nested-CV run:
   - file: exp06d_rf_bin_nosspace_elementwise_reduced_nestedcv/best_params_cv1.json

3. Run 5-fold CV1 with those fixed params (no nested search) for
   internal sanity and to keep a CV1 baseline consistent with the RF run.
   - feature selection is performed on each outer-train only

4. Train a final RandomForestClassifier on ALL internal training rows.
...
5. External evaluation on Chandrasekaran dataset:
   ...
   - Run final RF model, produce predictions, metrics, ROC/PR curve data
...
exp06d_rf_out = MODEL_RESULTS / "exp06d_rf_bin_cosyne_nosspace_elementwise_reduced_nestedcv"
ext_out = RESULTS / "external_validation" / "external_validation_chandrasekaran_rf_cosyne"
ext_out.mkdir(parents=True, exist_ok=True)

best_params_path = exp06d_rf_out / "best_params_cv1.json"

external_base_path = INTERIM / "source_d_chandrasekaran" / "chandrasekaran_cleaned_data.csv"

cc_path = CC_FEATURES / "cc_features_a4_only.csv"
...
print(f"Dropped {before_n - len(ext_base)} / {before_n} Chandrasekaran rows overlapping training pairs.")
...
print("\n=== External evaluation (Chandrasekaran, RF baseline) ===")
...
metrics_ext_path = ext_out / "external_metrics_chandrasekaran_rf.csv"
...
roc_df.to_csv(ext_out / "external_roc_curve_chandrasekaran_rf.csv", index=False)
pr_df.to_csv(ext_out / "external_pr_curve_chandrasekaran_rf.csv", index=False)
pr_thr_df.to_csv(ext_out / "external_pr_thresholds_chandrasekaran_rf.csv", index=False)
...
cm_df.to_csv(ext_out / "external_confusion_matrix_chandrasekaran_rf.csv")

print("\n=== External evaluation script (RF, Chandrasekaran) DONE ===")