#!/usr/bin/env python3
"""
** Experiment: exp06c_lgbm_bin_sspace_elementwise_reduced_nestedcv_bliss005_cv2 HALO-CV2s ** 
Reduced elementwise features (exp05c)
Brute-force CV2 (strain + drug-pair disjoint), repeated for 5 folds
Nested CV with inner StratifiedGroupKFold (group = drug pair)
No feature importance saved
Saves: per-fold metrics, mean±std summary, aggregated confusion matrix,
       cv2_info_fold{k}.json describing each brute-force split.
"""
...
def main():
    ...
    filtered_path = Path(
        ...
        "elementwise_features_filtered_cv2.csv"
    )
