#!/usr/bin/env python3
"""
scripts/other_analysis/shap_importance_analysis.py

Adds SHAP-based feature importance to complement HALO's existing
gain-based LightGBM importance (exp06d), and reports agreement
...
metrics_ext_path = ext_out / "external_metrics_acdb.csv"
...
exp06d_out = MODEL_RESULTS / "exp06d_lgbm_bin_nosspace_elementwise_reduced_nestedcv"
gain_importance_path = exp06d_out / "feature_importances_cv1.csv"  # your existing averaged gain file
out_dir = RESULTS / "other_analysis"
...
if isinstance(shap_values, list):
    sv = shap_values[1]  # positive class = synergy, matches pos_idx_final convention
else:
    sv = shap_values
...
shap_path = out_dir / "shap_feature_importance_final_model.csv"
shap_df.to_csv(shap_path, index=False)
print("Saved SHAP feature importances to:", shap_path)

# ==========================
# 2) Compare against existing gain-based ranking
# ==========================
gain_df = pd.read_csv(gain_importance_path)  # has 'feature', 'importance_gain', etc.
gain_df = gain_df.sort_values("importance_gain", ascending=False).reset_index(drop=True)
gain_df["gain_rank"] = np.arange(1, len(gain_df) + 1)

merged = shap_df.merge(
    gain_df[["feature", "importance_gain", "gain_rank"]],
    on="feature",
    how="inner",  # only features present in both selected sets
)

# Top-K overlap (K = 20, adjust as needed)
K = 20
top_shap = set(shap_df.head(K)["feature"])
top_gain = set(gain_df.head(K)["feature"])
overlap_k = len(top_shap & top_gain)

# Spearman rank correlation across all jointly-ranked features
rho, pval = spearmanr(merged["shap_rank"], merged["gain_rank"])

summary = {
    "n_features_compared": int(len(merged)),
    "top_k": K,
    "top_k_overlap": int(overlap_k),
    "top_k_overlap_frac": float(overlap_k / K),
    "spearman_rho": float(rho),
    "spearman_pval": float(pval),
}
print("\n=== SHAP vs. gain-based importance agreement ===")
print(summary)

merged.to_csv(out_dir / "shap_vs_gain_comparison.csv", index=False)
pd.DataFrame([summary]).to_csv(out_dir / "shap_vs_gain_summary.csv", index=False)
print("\nSaved comparison table and summary to:", out_dir)
