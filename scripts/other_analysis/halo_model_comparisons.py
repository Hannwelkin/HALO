...
EXP_DIRS = {
    ...
    "M4":   MODEL_RESULTS / "exp09d_lgbm_bin_nosspace_elementwise_reduced_simplecv",
}
...
# 1) Load M4's single-split metrics (NOT k-fold -- one row, one test split) 
def load_m4_metrics(m4_dir):
    """
    M4 uses a single stratified train/test split (no CV, no pair grouping -- see
    manuscript Table/Fig 2A: cv_scheme = "standard"). exp09d now writes this
    directly to metrics_test.csv (one row), so we just read it -- no more log parsing.
    Returns a dict of {metric_name: value}.
    """
    df = pd.read_csv(m4_dir / "metrics_test.csv")
    if len(df) != 1:
        raise ValueError(
            f"Expected exactly 1 row in M4's metrics_test.csv (single split), got {len(df)}."
        )
...
# 3) DeLong's test for comparing two ROC-AUCs directly from raw predictions 
...
# 6) Column-name mappings for models whose CSVs use short names (no "_test" suffix)
# HALO and M3's metrics_per_fold_cv1.csv use "roc_auc_test", "accuracy_test", etc.
# M1 and M2's metrics_per_fold*.csv use the short form: "roc_auc", "accuracy", etc.
# (both come from the same fold_results.append(dict(roc_auc=..., accuracy=...)) pattern).
M1_METRIC_MAP = {
...
M2_METRIC_MAP = dict(M1_METRIC_MAP)  # identical mapping, same column-naming convention
...
if __name__ == "__main__":
    ...
    # M3: paired test vs HALO (leakage comparison)
    ...
    # M2: paired test vs HALO 
    m2 = pd.read_csv(EXP_DIRS["M2"] / "metrics_per_fold_cv1.csv")
    print("\n" + "=" * 60)
    print("HALO vs M2 (paired -- same cv1 scheme)")
    print("=" * 60)
    for metric in METRICS:
        m2_col = M2_METRIC_MAP[metric]
        compare_models(halo[metric], m2[m2_col], paired=True,
                        label_a="HALO", label_b="M2", metric=METRIC_LABELS[metric])

    # M4: one-sample test vs HALO (M4 is a SINGLE split, not k-fold) 
    m4 = load_m4_metrics(EXP_DIRS["M4"])
    print("\n" + "=" * 60)
    ...
    for metric in METRICS:
        if metric in m4:
            ...
            print(f"WARNING: '{metric}' not found in M4's metrics_test.csv columns: {list(m4.keys())}")
